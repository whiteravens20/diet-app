...
/**
 * Curation queue card (Phase C — ingredient-name drafts only).
 *
 * Surfaces three things on `/admin`:
 *   - the "Generate drafts" button + runner progress strip
 *   - a paginated list of ingredient-name drafts, filterable by status
 *   - inline edit / approve / reject / delete per row
 *
 * Recipe drafts ride this same card in Phase D via a "kind" tab strip;
 * the wire-up here already plumbs the `kind` state so adding the second
 * tab will be a small follow-up.
 */
      ...
      <CardContent className="space-y-4">
        ...
        {data === null ? (
          ...
          <ul className="space-y-2">
            {data.items.map((d) => (
              <DraftRow key={d.id} draft={d} onChanged={onChanged} />
            ))}
          </ul>
        )}
...
function DraftRow({
  ...
  const onSave = () => {
    void handle(() => adminApi.patchIngredientNameDraft(draft.id, draftSuggestions))
  ...
  return (
    <li className="rounded-md border border-border p-3 text-sm">
      ...
      {draft.localeReviews.length > 0 && (
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          {draft.localeReviews.map((r) => (
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        {editing ? (
          <>
            <Button size="sm" disabled={working} onClick={onSave}>{t('save')}</Button>
