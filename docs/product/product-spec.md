...
| ~~F15~~ | **Inventory (pantry-aware planning)** — per-profile stock of ingredients the user actually has: quantity + unit + optional best-before. **Manual only**: the user owns what enters and leaves the pantry; the application never auto-decrements when meals are cooked and never auto-adds the full shopping check-off. The pantry's role is to surface real-world *leftovers* (over-bought a 700 g pack for a recipe that needed 650 g → 50 g lives in the pantry, ready for the next plan to lean on). The optimiser weights recipes by `coverage = covered_ingredient_mass / required_ingredient_mass`. Shopping lists pre-fill `alreadyHaveQuantity` from the pantry so the buy column is reduced by what's already on hand. **Shipped 2026-06**: `InventoryItem` table, `/inventory` CRUD page (profile picker, category-grouped list, inline qty/best-before edit, add-form with ingredient typeahead), optimiser coverage scoring on `generate`/`regenerate`/`regenerateDay`, swap-pool re-rank (deterministic random path + `aiSwapMeal` + `aiSuggestIngredient`) — AI features see pantry-friendly candidates first via pool pre-sort (engine builds the pool, AI ranks), never the pantry directly. `respectInventory` boolean on every relevant request schema (`GeneratePlanRequest`, `SwapMealRequest`, `AiSwapMealRequest`, `AiSuggestIngredientRequest`) with **UI toggles**: a *Prefer recipes that use my pantry* checkbox on the plan-generation form (defaults on) and a page-level *Bias swaps toward what's in my pantry* checkbox on the meal-plans page that applies to every deterministic swap AND every AI swap on every plan card. Both default-on, both togglable any time. There is **no Delete-meal button and no Mark-eaten button** — inventory is not a consumption ledger; it tracks leftovers, and the user maintains it directly from `/inventory`. Anti-monotony rotation, shopping-leftover auto-add and small UI polish are parked under [[F15.1]]. |
| F15.1 | **Inventory follow-ups** — two correlated jobs and a small polish pass, all gated on F15's manual baseline. **(a) Shopping over-buy → auto-leftover to pantry.** Close the loop between the shopping list and the pantry. Each shopping-list row gains an inline numeric **"bought"** input next to the buy quantity, pre-filled with the `toBuyQuantity` value so the default behaviour is unchanged. If the user enters a higher number (the list said 550 g, the user bought a 600 g pack) and then checks the item off, the diff (`bought − toBuy`) auto-creates a leftover `InventoryItem` for the profile in the item's unit (`50 g` of that ingredient lands in `/inventory`). The new pantry row carries a `source = 'SHOPPING_LEFTOVER'` marker so the `/inventory` page can show a small "from shopping" chip — and like every other inventory row it is freely **editable or deletable** from `/inventory`. **Idempotency**: the leftover-add only fires on the `false → true` check-off transition, so unchecking and re-checking does not double-add. Editing `bought` while the item is already checked posts a corrective patch — the existing leftover row is adjusted (or deleted when the diff drops to zero) in the same transaction. *Schema*: `purchasedQuantity Float?` on `ShoppingListItem` (null = "matches toBuy, no leftover"), `source` enum on `InventoryItem` (`MANUAL` default; `SHOPPING_LEFTOVER` for the auto-flow). *API*: `PATCH /shopping-lists/:listId/items/:itemId` accepts `purchasedQuantity`. **(b) Anti-monotony rotation for inventory-biased generation.** After **N** consecutive inventory-biased rounds on the same profile (default `N=5`, configurable per profile via `inventoryBiasResetEvery`) the optimiser deliberately drops the pantry bias for the next round so the menu doesn't collapse onto the same handful of pantry-friendly recipes; the round after that, the bias resumes. Scoped to plan-level paths (`generate` / `regenerate` / `regenerateDay`) — per-meal swap doesn't increment the streak (one swap isn't a "round"). *Schema*: `Profile.inventoryBiasStreak Int @default(0)` transient counter, `ProfilePreference.inventoryBiasResetEvery Int @default(5)` threshold (`0` disables the reset and always biases when the pantry is non-empty). *Service*: `resolveInventoryBias` reads the streak and threshold before deciding to compute a coverage map; a sibling `updateInventoryStreak` increments on apply, zeroes on skip, runs best-effort post-success. **(c) UX polish (small, non-blocking)** — a "from pantry" chip on shopping items whose `alreadyHaveQuantity > 0` (tooltip lists the source pantry rows); a numeric input on the profile preferences card for `inventoryBiasResetEvery`; a red urgency pill on inventory rows whose `bestBefore ≤ today + 2 days` (composes with [[F17]]'s *use-up-by* day flag when that lands). Composes with [[F15]]'s coverage bias — (a) feeds the pantry, (b) keeps the bias from over-fitting, (c) makes the connection visible. |
