...
| ~~F17~~ | **Advanced meal-plan options** — **Shipped 2026-06.** the plan generator gets a "Show advanced options" disclosure (collapsed by default; the basic flow stays date-range + meal count + diet type, so a new user is never confronted with these knobs). When expanded the user can: (a) **override the meal count per day** — e.g. 5 meals on Mon/Wed/Fri training days, 3 meals on rest days — while the global meal count remains the fallback; (b) **override the daily calorie target per day** — refeed/training surplus, rest-day deficit — so a single weekly plan can hold a real periodisation pattern instead of being flat; (c) tag a day as **rest** or **training** so it automatically targets fewer / more calories (±15% of the base target, overridable by an explicit per-day calorie value) and so the tag carries semantic meaning for dashboard/stats and the optimiser knows when a surplus is intentional; (d) **skip** a day entirely (still inside the range but the engine produces no meals — for restaurant nights, travel, social events) and the shopping list excludes it; (e) **lock a slot** on a specific day to a chosen recipe before generating, so the rest of the plan optimises around it. Two cross-day knobs round the panel out: a **variety floor** (`max_repeats_per_recipe` across the window, default 2) so the optimiser can't collapse onto two recipes when constraints are loose, and a **cook-time budget per day** (hard cap in minutes) so busy weekdays only get quick recipes. Schema impact: a single `overrides` JSON column on `meal_plan_days` is enough — none of these settings need first-class tables because they only feed the generator and the rendered plan. Per-day overrides surface as a compact editable list inside the disclosure with one row per planned date; each row falls back to "use plan default" when untouched. Re-roll honours overrides: regenerating a single day uses that day's overrides, regenerating the whole plan re-applies the per-day list. Composes with [[F13]] favorite-sets (a set can be dropped onto a specific day index from the advanced view, e.g. *"Set 1 → Monday, Set 2 → Wed–Fri"*), [[F15]] inventory (a *use-up-by* day flag biases the optimiser toward recipes that consume expiring inventory on the chosen date) and [[F16]] admin panel (operator-level default for `max_repeats_per_recipe` lives in the config surface). **Forward-composes with [[F22]] flexible meal plans** (near-term follow-up): F17 is *generator-time* — it shapes the periodisation pattern the optimiser produces; F22 is *edit-time* — it mutates an already-generated plan day-by-day without rerolling. The two operate on different layers and **neither gates the other** — F17 ships and is useful with no F22, and F22 ships on top of plans generated with or without F17 overrides. The one deliberate coupling: F17's per-day calorie target and rest/training tags persist on the day row (the `overrides` JSON column), not as transient generator inputs, **specifically so F22's quantity rebalancer can read the per-day target at edit time** (F22 (c) falls back to the plan-level target only when no F17 per-day override exists). Designing F17 now with that persistence contract in mind means F22 lands later as an additive read of the same column — no migration churn, no re-plumbing. **As shipped:** the per-day calorie override is written to the existing `meal_plan_days.calorieTarget` column (the semantics — meal count, rest/training, skip, cook-time budget, use-up-by, locks — live in the new `overrides` JSON column); a **rest/training tag auto-shifts the day's target ±15%** (engine `dayTypeCalorieTarget`, overridable by an explicit per-day calorie value); the variety floor is **opt-in** (blank = no cap) rather than a silent default of 2, so the basic flow's output is unchanged on the small curated DB; the use-up-by bias reuses the F15 coverage engine, scored against stock whose `bestBefore ≤ the flagged date`. |
...
| F22 | **Flexible meal plans** — composes with [[F17]] advanced options but lives one layer above it: F17 lets the *generator* hold a periodisation pattern, F22 lets the *user* edit an already-generated plan day-by-day without rerolling. Three correlated jobs. **(a) Swap a planned slot to any favourite, diet-type be damned.** The favourite-picker on a planned meal currently filters to recipes matching the plan's `dietType` (and meal-type, allergens, etc.); F22 adds a *"Show all my favourites"* toggle inside the favourite-swap dropdown that drops the diet-type filter (allergens + meal-type stay enforced — eating peanuts when the profile flags an allergy is never one click away). The replaced meal carries a small chip indicating the user overrode the diet bias, so the dashboard "% of plan on-diet" stat in [[F12]] stays honest. **(b) Add a custom user-authored meal.** Each day row gets a *"+ Add custom meal"* button alongside the existing meal swap actions; the modal asks for a free-text name, the meal-type slot, and required macro inputs (`kcal`, `protein g`, `fat g`, `carbs g`, `servings` defaulting to 1) — the user owns the numbers because we are not the ones who will recompute them from a deterministic ingredient list. A custom meal carries `source='USER_CUSTOM'` on the `PlannedMeal` row plus the supplied macros frozen on the row itself (no `Recipe` row created — this is plan-local data, not catalogue growth; [[F18]]'s `AI_USER` promotion path is for AI-drafted recipes that could become curated, not for ad-hoc meals the user typed once). Custom meals appear in the plan exactly like any other meal (chip strip with `P/F/C/kcal`, modal showing the user-entered numbers + a *"Custom meal — macros entered by you"* notice instead of the recipe view; no shopping-list line written because we don't know the ingredients). **(c) Auto-rebalance quantities, never swap meals.** After any edit that changes the day's total — swap, custom add, ingredient quantity edit, ingredient substitution from [[F20]] (b) — the engine recomputes the day's macros and, if the day deviates from its calorie target (per-day target if [[F17]] (b) is set, otherwise the plan-level target) by more than ±10%, **scales the per-serving quantities of the *unchecked, non-custom* meals on that day** to pull the day back into the ±10% window. Quantity-only rebalancing means the menu the user already approved stays intact (we never reach in and replace one of their meals to fix a calorie gap they introduced); the engine simply adjusts how much of each remaining meal they should eat. Custom meals are pinned (the user owns those macros — scaling them would invalidate the inputs they typed) and eaten meals are pinned (see (d)). If quantity-only rebalancing cannot close the gap inside the ±10% window (e.g. the user added a 1200 kcal custom meal on top of a 2000 kcal target), the recompute lands at the closest feasible distribution and a yellow chip *"Day over target — consider editing"* surfaces on the day row; we don't drop meals automatically. **Week-aware**: a *"Rebalance the rest of the week to compensate"* checkbox in the recalculation toast (default off) extends the redistribution across all unchecked, non-custom meals in the same plan week, sharing the surplus / deficit proportionally so each remaining day ends within its own ±10% window where possible. Without that opt-in we only touch the edited day. **(d) Mark-eaten checkmarks.** Every planned meal gains a checkbox (`PlannedMeal.eatenAt DateTime?`); a checked meal is excluded from the rebalance set on its day *and* on the week (we already-ate-it, scaling it retroactively is meaningless). The shopping list ignores `eatenAt` (the list is generated once at plan start; ticking a meal as eaten does not retroactively edit the list — that's [[F15.1]] (a)'s job, decoupled). A small *"Eaten"* badge replaces the swap actions on a checked meal; unchecking restores them and re-includes the meal in subsequent rebalances. **Information surface**: every rebalance writes one toast (*"Day macros updated — protein 142 → 138 g, lunch quantity scaled to 1.15×"*) and a small "auto-balanced" chip on the affected meals that clears on the next manual edit; an *"Undo last rebalance"* link in the toast restores pre-edit quantities for 10 s. *Schema*: `PlannedMeal { source: enum('CATALOGUE' | 'USER_CUSTOM') @default('CATALOGUE'), customName: String?, customMacros: Json?, eatenAt: DateTime?, quantityScale: Float @default(1.0), lastRebalanceAt: DateTime? }`. *Engine*: pure rebalancer in `apps/api/src/engine/rebalance.ts` (signature `rebalanceDay(day, lockedMealIds, target)` returns the new per-meal `quantityScale` map plus a `feasibility: 'in-window' | 'best-effort'` flag); unit-tested across the full ±10% / ±20% / impossible-gap matrix. *API*: `PATCH /meal-plans/:id/meals/:mealId/eaten` toggles `eatenAt`; `POST /meal-plans/:id/days/:date/custom-meal` adds one; `POST /meal-plans/:id/rebalance` body `{ scope: 'day' \| 'week', date? }` for explicit re-runs (the auto-rebalance also calls this server-side after any qualifying edit, single-flight per plan). *AI awareness*: when [[F20]] (b) ingredient-swap accepts a substitute, the same rebalance pipeline runs; AI-suggested swaps that the rebalancer can't keep inside ±10% surface a stronger warning before the user accepts. **Out of scope this row** (parked, intentionally — F22 is already wide): per-meal quantity input as a manual override (a future *"I had a smaller portion"* action that nudges `quantityScale` without re-triggering the day rebalance); micronutrient-aware rebalancing (the ±10% window is per-meal kcal, not per-macro, because chasing all four macros simultaneously with one knob is under-constrained); custom-meal photos / recipe entry (custom = macros only; if the user wants a real recipe row, [[F20]] (c) "Draft with AI" is the path); week-level visual *"calorie debt"* counter on the dashboard (the toast tells the story for v1; the chart belongs in a future dashboard pass). |
...
`meal_plan_days`), **flexible meal plans** (F22: edit an
already-generated plan day-by-day without rerolling — swap a slot to any
favourite, add custom user-macro meals, mark meals eaten, and quantity-only
auto-rebalance back to the ±10% calorie window; sits one layer above F17 and
reads its persisted per-day targets, neither feature gating the other),
improved ingredient-reuse optimisation, **curation queue**
